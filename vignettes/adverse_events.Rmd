...
There are three distinct ways to present these grade tables. The most common one is the table of maximum grades, where the variant="max" is used by default. Alternatively, we can present the table by grade superior or equal to a given grade, i.e., variant="sup", or we can present all grades for all patients, where each proportion represents the number of patients who experienced a given grade at least once.????
...
## Table of AE Grade Superior or Equal to Given Grade

```{r}
ae_table_grade(df_ae=ae, df_enrol=enrolres, arm=NULL, variant="sup") %>% 
  as_flextable(header_show_n=TRUE) %>%
  add_footer_lines("Percentages reflect the proportion of patients whose AE grade was superior or equal to the indicated grade.") %>%
  fontsize(size = 8, part = "all")
```
