...
-   The second variant, **`variant = "sup"`**, summarises the grades greater than or equal to a specified grade per patient where n is the number of unique patients who have experienced at least one AE of grade greater than or equal to X.

-   The third variant, **`variant = "eq"`**, presents all grades for all patients. In this table, each proportion represents the number of patients who experienced a given grade at least once where n is the number of unique patients who have experienced at least one AE of grade X.
...
The ae_table_soc() function generates System Organ Class (SOC)–level summary tables of adverse events (AEs). These tables provide an overview of AE frequencies and grades, either by SOC, by Preferred Term (PT), or stratified by treatment arm. They are useful for identifying patterns of toxicity across organ systems and comparing safety profiles between study arms.

There are several common ways to present SOC tables. Below are the most frequently used formats:

**AE_SOC1: Table of AE grades by SOC**
...
**AE_SOC2: Table of AE grades by SOC and AE terms**

Displays AE grades by SOC and Preferred Term (PT), providing more detailed information about which specific events contribute to the frequencies within each SOC.
...
**AE_SOC3: Table of AE grades by AE terms only (filtered for serious AEs)**

Summarises AE grades by Preferred Term only, filtered for Serious Adverse Events (SAEs). To create this table, filter the AE dataset beforehand to include only serious events.
...
**AE_SOC4 / AE_SOC5: SOC tables stratified by treatment arm (without total column)**

Displays AE grades by SOC, stratified by treatment arm, without a total column. This format allows direct comparison of AE profiles between study arms.
...
**AE_SOC6: Table of SOC and AE terms, all grades combined, stratified by arm**
