...
#' may slightly exceed `n` (up to `n + max(block_sizes) - 1`).
#'
...
#' `max(block_sizes)/length(arms)` slots. This is a conservative bound and is typically
  ...
  strata %>%
    ...
    map(~{
      blockrand::blockrand(n = n,
                           ...
                           stratum = .x,
                           id.prefix = paste0(.x, " - "),
                           block.sizes = block_sizes/length(arms),
                           ...)
    }) %>%
    list_rbind() %>%
...
.check_blocks = function(block_sizes){
  ...
  if(length(odds)>0){
    cli_warn(c(
      "Block sizes should be even for treatments to be balanced. {.arg block_sizes}={.val {odds}} is even."
    ),
...
.check_arms = function(block_sizes, arms){
  ...
  if(any(wrong)){
    cli_abort(c(
      "Some {.arg block_sizes} are not divisible by the number of treatment arms",
      i="There are {.val {length(arms)}} treatment arms: {.val {arms}}",
      i="{.arg block_sizes} {.val {block_sizes[wrong]}} {cli::qty(sum(wrong))} {?is/are} not
